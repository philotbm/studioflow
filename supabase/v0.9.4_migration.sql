-- v0.9.4 Memberships / Packs Foundation — scope correction migration.
--
-- Removes the account-lifecycle override from sf_check_eligibility. The
-- Memberships / Packs Foundation release is deliberately narrowed to
-- entitlement truth only:
--
...
-- only. Any future "account lifecycle as access gate" behaviour is out
-- of scope for v0.9.4 and belongs in a dedicated release.
--
-- Idempotent — CREATE OR REPLACE FUNCTION replaces any prior definition,
-- so this migration can be re-applied safely.
