...
/**
 * v0.9.4 Memberships / Packs Foundation.
 *
 * This module is a THIN, TS-only derivation layer. It does not replace
 * `sf_check_eligibility` or `v_members_with_access` — the DB remains the
 * sole source of booking-gating truth. What this module provides is a
 * single consolidated "commercial status" snapshot that the operator UI
 * can read off one object instead of assembling it from three separate
 * fields (plan_type, plan_name, credits).
 *
 * v0.9.4 scope rule (deliberately narrow — see release notes):
 *
 *   unlimited plan                 → bookable
 *   positive credits (pack/trial)  → bookable
 *   otherwise                      → not bookable
 *
 * Account lifecycle (active/paused/inactive) is NOT part of the access
 * decision in this release. It stays on the Member record as
 * informational metadata and is intentionally not read by any function
 * here. Any future "account status as access gate" behaviour is a
 * separate release.
 */
...
export type MembershipSummary = {
  ...
  tone: MembershipTone;
  /**
   * Entitlement-only bookability: true when plan is unlimited OR
   * credits > 0. This is the v0.9.4 commercial rule — the server's
   * `bookingAccess.canBook` remains authoritative for actual gating.
   */
  canBook: boolean;
};
...
/**
 * Entitlement-only bookability — the v0.9.4 rule. Unlimited plans are
 * always bookable; credited plans are bookable when balance > 0; drop-in
 * (no persistent entitlement) is not bookable.
 */
function canBookFor(member: Member, status: MembershipStatus): boolean {
  if (status === "unlimited_active") return true;
  if (status === "drop_in_only") return false;
  return (member.credits ?? 0) > 0;
}
 ...
 * the Member's own values. Callers should NOT use MembershipSummary for
 * booking-gating decisions — use `member.bookingAccess` (the server's
 * truth) for that. MembershipSummary is a presentation projection.
 */
export function summariseMembership(member: Member): MembershipSummary {
  ...
  return {
    ...
    tone: toneFor(status),
    canBook: canBookFor(member, status),
  };
}
