...
**Tenant-scoped queries must call `scopedQuery()` from `src/lib/db.ts`, not `getSupabaseClient()` from `src/lib/supabase.ts`.** Today the two helpers are identical (v0.21.0.5 is a mechanical rename with zero behaviour change). After M3 (v0.22.0), `scopedQuery` will auto-apply `.eq('studio_id', current_studio_id())` to every `.from(<tenant_scoped>)` call, and call sites that bypass it will leak cross-tenant data. Tenant-scoped tables: `members`, `staff`, `classes`, `class_bookings`, `booking_events`, `credit_transactions`, `plans`, `purchases`, and views derived from them (`v_members_with_access`). Exceptions that must keep using `getSupabaseClient()` directly: auth-row reads (the `staff` self-read in `src/lib/auth.ts` and `src/proxy.ts`, the auth-callback / auth-claim member lookups), the `studios` table itself (lands in M3), and intentional cross-tenant operations — the Stripe webhook is the canonical example, because it has no caller session and derives `studio_id` from event metadata instead. Intentional exceptions MUST carry an inline comment naming the reason. RPC sites carry `TODO(M3)` markers and will need explicit `p_studio_id` parameters once the underlying PL/pgSQL is studio-scoped. See `docs/adr/0001-multi-tenancy.md` Decision 6.
